...
export class PhysicalCountService {
  ...
  async updateItemCount(
    ...
    try {
      ...
      // Actualizar item
      const updatedItem = await this.repository.updateItemCount(itemId, {
  ...
  async getCountItems(countId: string, companyId: string) {
    try {
      ...
      const items = await this.repository.getCountItemsWithLocation(countId);

      // Calcular estadísticas
      const stats = {
        totalItems: items.length,
        itemsCountedFully: items.filter((i: any) => i.countedQty > 0).length,
        itemsNotCounted: items.filter((i: any) => i.countedQty === null || i.countedQty === 0).length,
        itemsWithVariance: items.filter((i: any) => i.countedQty !== null && i.countedQty !== i.systemQty).length,
        totalSystemQty: items.reduce((sum: number, i: any) => sum + Number(i.systemQty), 0),
        totalCountedQty: items.reduce((sum: number, i: any) => sum + (i.countedQty || 0), 0),
      };
      ...
      const itemsWithVariance = items
        ...
        .map((i: any) => ({
          ...
          variancePercent: Math.round(((Number(i.countedQty) - Number(i.systemQty)) / Number(i.systemQty)) * 10000) / 100,
        }));
  ...
  async completeCount(
    ...
    try {
      ...
      const items = await this.repository.findItemsByCountId(countId);

      if (items.length === 0) {
        throw new AppError(400, 'Cannot complete count with no items');
      }

      const uncountedItems = items.filter((i: any) => i.countedQty === null || i.countedQty === 0);
      ...
      const updatedCount = await this.repository.updateCount(countId, {
        status: 'COMPLETED',
  ...
  async getVarianceSummary(countId: string, companyId: string) {
    try {
      const count = await this.repository.findCountWithItems(countId, { NOT: { countedQty: null } });
      ...
      const variances = count.countItems
        .map((item: any) => {
          ...
          return {
            ...
            variance,
            variancePercent: Number(item.systemQty) > 0 ? Math.round(((variance / Number(item.systemQty)) * 10000) / 100) : 0,
      ...
      const summary = {
        ...
        itemsOK: count.countItems.filter((i: any) => Number(i.countedQty) === Number(i.systemQty)).length,
        itemsWithVariance: variances.length,
        totalSystemQty: count.countItems.reduce((sum: number, i: any) => sum + Number(i.systemQty), 0),
        totalCountedQty: count.countItems.reduce((sum: number, i: any) => sum + Number(i.countedQty), 0),
        totalVariance: variances.reduce((sum: number, v: any) => sum + v.variance, 0),
        overages: variances.filter((v: any) => v.variance > 0).reduce((sum: number, v: any) => sum + v.variance, 0),
        shortages: Math.abs(variances.filter((v: any) => v.variance < 0).reduce((sum: number, v: any) => sum + v.variance, 0)),
      };

      return {
        summary,
        variances: variances.slice(0, 20),
      };
    } catch (error: any) {
      if (error instanceof AppError) throw error;
